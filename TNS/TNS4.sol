...
contract TNS is Ownable, IERC165 {
    ...
    function encodeAddrField(string calldata name, uint256 coinType) pure external returns (bytes memory) {
        if (coinType < 0x100000000) {
            ...
            return abi.encodePacked(KIND_FLAG_ENCODED, abi.encode(name, abi.encodeCall(IAddressResolver.addr, (0, coinType))));
        }
    }
    ...
    function lookupTest(string calldata a, string calldata b) external pure returns (bytes memory) {
        return dns_from_name(abi.encodePacked(a, '.', b));
    }
	...
	function lookupOnchain(bytes32 node, bytes[] memory values) internal view returns (KV[] memory kv) {
		unchecked {
			...
			for (uint256 i; i < values.length; i += 1) {      
                bytes memory f = getTiny(SLOT_FIELDS + i);
                uint256 kind = uint8(f[0]);
                (bool ok, bytes memory v) = resolver.staticcall(calls[i]);
                if (ok) {
                    v = decodeResult(kind, v);
    ...
    function decodeResult(uint256 kind, bytes memory v) internal pure returns (bytes memory) {
        if ((kind & KIND_FLAG_NO_DECODE) != 0) { // addr(), pubkey()
            uint256 p;
            uint256 e;
            assembly { 
                p := add(v, 32)
            ...
            return abi.decode(v, (bytes)); // addr(x), text(), contenthash()
        }
    }
